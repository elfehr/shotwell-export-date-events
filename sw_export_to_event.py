    ...
    for row in crs:
        if row[1] is not None:
            ...
            timestamp_crs.execute(
                "select exposure_time from PhotoTable where event_id=? order by exposure_time limit 1",
                (row[0],),
            )
            try:
                event_ym = datetime.fromtimestamp(timestamp_crs.fetchall()[0][0]).strftime(
                    "%Y-%m"
                )
                print("Identified event date", event_ym)
            except IndexError:
                print("Seems no files present in DB that match the event")
                continue
            event_dir = (
            ...
            photo_crs.execute(
                "select filename from PhotoTable where event_id=?", (row[0],)
            )
            copy_cnt = 0
            exist_cnt = 0
            for photo in photo_crs:
                ...
                copy_needed = False
                if comp_sha:
                    copy_needed = not os.path.exists(filename) or shafile(
                        filename
                    ) != shafile(photo[0])
                else:
